...
| **M1** | 从上游参考仓库 **快照复制** scripts + hooks + harness 骨架到 `scripts-bundle/`、`templates/` | **已完成**：`scripts-bundle/`、`templates/cursor`（含已修正文案的 `hooks.json`）、`templates/harness-skeleton/`、`templates/*.example.md`、`install/apply-to-repo.sh`、`skill/SKILL.md`（技能名 **`mmchong-init`**） |
